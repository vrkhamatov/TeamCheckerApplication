    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            switch (messageText) {
                ...
                case "Войти в группу" -> sendMessage(chatId, "Введите номер комнаты, в которую хотите войти");
                case "Создать группу" -> {
                    try {
                        String chatIdStr = String.valueOf(chatId);

                        sendMessage(chatId, DbHelper.createPost(chatIdStr, update.getMessage().getChat().getUserName()));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
